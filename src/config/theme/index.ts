...
const THEME_REGISTRY = {
  ...
  // Onboarding-only theme (not shown in theme picker)
  'VaNiTheme': VaNiTheme,
} as const;
...
export function getThemeMetadata(themeName: ThemeName) {
  ...
  return {
    name: themeName,
    displayName: (themeConfig as any).displayName || themeName,
    description: (themeConfig as any).description || 'A beautiful theme for your application',
    category: (themeConfig as any).category || 'general',
    author: (themeConfig as any).author || 'ContractNest',
    version: (themeConfig as any).version || '1.0.0',
    tags: (themeConfig as any).tags || [],
    preview: (themeConfig as any).preview || null,
...
export function meetsAccessibilityStandards(backgroundColor: string, textColor: string, level: 'AA' | 'AAA' = 'AA'): boolean {
