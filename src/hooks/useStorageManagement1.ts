...
export const useStorageManagement = () => {
  ...
  const uploadFile = async (file: File, category: string): Promise<StorageFile | null> => {
    ...
    try {
      ...
      console.log(`Uploading file to ${isLive ? 'Live' : 'Test'} environment`);
      
      const response = await api.post(API_ENDPOINTS.STORAGE.FILES, formData, {
        headers: {
  ...
  const uploadMultipleFiles = async (files: File[], category: string): Promise<boolean> => {
    ...
    try {
      ...
      const response = await api.post(`${API_ENDPOINTS.STORAGE.FILES}/multiple`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
