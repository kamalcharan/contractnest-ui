...
//
// When Tax Settings say "no tax" the card still renders, muted and marked
// "Tax not configured", and opens Tax Settings instead of the records page —
// hiding it entirely left users wondering where GST had gone. Renders
...
const GstMonthCard: React.FC<GstMonthCardProps> = ({ side }) => {
  ...
  if (isNoTax) {
    return (
      <button
        type="button"
        onClick={() => navigate('/settings/tax-settings')}
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <span className="w-9 h-9 rounded-xl flex items-center justify-center flex-none"
                style={{ backgroundColor: `${dim}1a` }}>
            <Receipt size={16} style={{ color: dim }} />
          </span>
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: dim }}>
              GST · Tax not configured
            </p>
            <p className="text-sm mt-0.5" style={{ color: dim }}>
              Tax Settings are set to “No tax”. Set up GST to start tracking it here.
            </p>
          </div>
        </div>
