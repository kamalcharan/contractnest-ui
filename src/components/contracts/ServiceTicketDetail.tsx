        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {/* B3.6 — public service report (token link, printable) */}
              {(ticketDetail as any)?.report_token && (
                <a
                  href={`/report/service/${(ticketDetail as any).report_token}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-opacity hover:opacity-90"
                  style={{ backgroundColor: `${colors.brand.primary}12`, color: colors.brand.primary }}
                  title="Open the shareable service report"
                >
                  View Report
                </a>
              )}
