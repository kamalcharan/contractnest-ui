// src/pages/public/aimaturity.tsx
import React, { useEffect, useState } from 'react';
import { AIMaturityProvider } from '@/components/Leads/AIMaturityAssessment/AIMaturityContext';
import AIMaturityAssessmentMain from '@/components/Leads/AIMaturityAssessment';
import { ErrorBoundary } from '@/components/ui/error-boundary';
import { useTheme } from '@/contexts/ThemeContext';
import { analyticsService } from '@/services/analytics.service';
import { captureError } from '@/config/sentry.config';

const AIMaturityPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initPage = async () => {
      try {
        console.log("AI Maturity Assessment page mounted");
        ...
        if (isMounted) {
          try {
            ...
            await Promise.race([
              analyticsService.trackWorkspaceEvent('ai_maturity_page_viewed', {
                theme: theme.mode,
        ...
        if (isMounted && window.Sentry) {
          window.Sentry.addBreadcrumb({
            category: 'navigation',
            message: 'Viewed AI Maturity Assessment page',
            level: 'info'
          });
        }
      } catch (error) {
        console.error("Error initializing AI maturity page:", error);
        if (isMounted) {
          try {
            captureError(error as Error, {
              tags: { component: 'AIMaturityPage', action: 'initialization' }
            });
          } catch (sentryError) {
  ...
  return (
    <ErrorBoundary onError={(error) => {
      ...
      try {
        captureError(error, {
          tags: { component: 'AIMaturityPage', action: 'render' }
        });
      } catch (sentryError) {
        console.error("Error logging to Sentry:", sentryError);
      }
      ...
      <div className={`min-h-screen ${theme.mode === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-gray-50 text-gray-800'}`}>
        <div className="container mx-auto px-4 py-8">
          ...
          <AIMaturityProvider>
            <AIMaturityAssessmentMain />
          </AIMaturityProvider>
          
          <footer className={`text-center text-sm mt-8 ${theme.mode === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
            <p>© {new Date().getFullYear()} Vikuna Technologies. This assessment provides a high-level overview and should not be considered a comprehensive audit.</p>
