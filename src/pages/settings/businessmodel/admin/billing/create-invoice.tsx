...
// Get tenant options from fake data
const tenantOptions = fakeTenants.map(tenant => ({
  id: tenant.id,
  name: tenant.name
}));

// Get plans for dropdown from fake data
const planOptions = fakePricingPlans
...
const CreateInvoicePage: React.FC = () => {
  ...
  useEffect(() => {
    if (tenantId) {
      ...
      const invoicesForTenant = fakeInvoices.filter(invoice => invoice.tenantId === tenantId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const tenantName = tenantOptions.find(t => t.id === tenantId)?.name || '';
    
    // Get plan name from options
    const planName = planOptions.find(p => p.id === planId)?.name || '';
