...
const BusinessPreferencesStep: React.FC = () => {
  ...
  // FIX: Changed from 'whatsapp_number' to 'business_whatsapp'
  const handleWhatsAppChange = (whatsapp: string) => {
    setWhatsappNumber(whatsapp);
    if (whatsapp) updateTenantField('business_whatsapp', whatsapp);
  };

  // FIX: Changed from 'whatsapp_country_code' to 'business_whatsapp_country_code'
  const handleWhatsAppCountryCodeChange = (code: string) => {
    setWhatsappCountryCode(code);
    updateTenantField('business_whatsapp_country_code', code);
  };
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // FIX: Added WhatsApp data to dataToSend (was missing!)
      if (whatsappNumber) {
        dataToSend.business_whatsapp = whatsappNumber;
        dataToSend.business_whatsapp_country_code = whatsappCountryCode;
      }
